...
const transformEditorState = (props: IMUIRichTextEditorProps) => {
...
> = (props, ref) => {
	...
	useEffect(() => {
		const editorState = transformEditorState(props)
	...
	const handleBeforeInput = (chars: string): DraftHandleValue => {
		...
		const currentLength = editorState.getCurrentContent().getPlainText('')
			.length
		return isGreaterThan(currentLength + 1, props.maxLength)
			? 'handled'
			: 'not-handled'
	}
	...
	const handlePastedText = (
		...
		const currentLength = editorState.getCurrentContent().getPlainText('')
			.length
		return isGreaterThan(currentLength + text.length, props.maxLength)
			? 'handled'
			: 'not-handled'
	}
