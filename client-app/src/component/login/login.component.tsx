...
import * as IdentityActions from 'store/identity/actions'
...
class Login extends React.PureComponent<IComponentProps, IComponentState> {
	...
	changeLanguage = (e: React.MouseEvent): void => {
		...
		LocaleActions.actionCreators.changeLanguage(this.props.dispatch, newLang)
	}
	...
	loginHandleClick = () => {
		...
		setTimeout(() => {
			LoaderActions.actionCreators.toggleLoader(this.props.dispatch, false)

			AlertActions.actionCreators.showAlert(
				this.props.dispatch,
				'success',
				new Date().toJSON()
			)
			IdentityActions.actionCreators.login(this.props.dispatch)
	...
	loginHandleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (
			...
			(validator.current?.errors as any)[target.name]
		) {
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			const err = validator.current?.errors as any
					...
					<Card classes={classList} raised>
						...
						<div className="dis-flex">
							<img
								alt="en"
								title="en"
								src={require('asset/image/uk.png')}
							...
							<img
								alt="vi"
								title="vi"
								src={require('asset/image/vietnam.png')}
								onClick={this.changeLanguage}
								height="32px"
								width="32px"
								className="p-t-1 p-b-1 p-l-1 p-r-1 hov-pointer login-lang-switch"
